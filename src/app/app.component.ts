...
export class AppComponent implements OnInit {
  ...
  deleteClient = async (client: any) => {
    await this.clientService.deleteClient(client.id, client).then(async data => {
      await this.getallClients()
      this.clientDialog = false
      this.messageService.add({ key: 'tst', severity: 'success', summary: 'Store Message', detail: 'Client has been deleted' });
    }).catch(err => {
      console.log("Error")
      console.log(err)
    })
